...
export default function ClientsContent() {
  ...
  const backButton = (
    ...
    >
      ...
      <LangWipe>{t(lang as Lang, "menuBack")}</LangWipe>
    </button>
  );
  ...
  const headerBlock = (
    <div>
      <h1
        style={{
          fontSize: "clamp(1.3rem, 2.5vw + 0.8rem, 2.2rem)",
          fontWeight: 700,
  ...
  return (
    ...
    >
      ...
      <main
        style={{
          padding: isMobile
            ? "calc(var(--safe-top) + 0.75rem) calc(var(--safe-right) + 0.75rem) calc(var(--mobile-nav-h) + 0.75rem) calc(var(--safe-left) + 0.75rem)"
      ...
      >
        ...
        {isMobile && <div style={{ marginBottom: "0.75rem" }}>{headerBlock}</div>}
        ...
        >
          <div
            ...
            style={{
              ...
              background: "var(--bg)",
              borderRight: "1px solid var(--border)",
              padding: "calc(var(--safe-top) + 1rem) 1rem 1rem 1rem",
              overflowY: "auto",
              display: "flex",
              flexDirection: "column",
              gap: "1rem",
      ...
      <style>{`
        @keyframes clientsDrawerSlideIn {
          from { transform: translateX(-100%); }
          to   { transform: translateX(0); }
        }
        .clients-drawer { animation: clientsDrawerSlideIn 0.22s ease; }
      `}</style>
    </div>
  );
}