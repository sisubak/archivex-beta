...
export default function Section({ section, active, isMoving }: Props) {
  const blurAmount = !active && isMoving ? 3 + moveIntensity(active, isMoving) * 0 : 0;
...
function moveIntensity(_active: boolean, _isMoving: boolean) {
  return 0;
}
...
function titleKeyFor(id: string): string {
  return `section${id.charAt(0).toUpperCase() + id.slice(1)}`;
}

function DefaultContent({ id }: { id: string }) {
  const { lang } = useApp();
  const translated = t(lang as Lang, titleKeyFor(id));
